...
- 上部にトランスポート/状態ヘッダー、中央にアレンジャー+エディタ、右にインスペクタ、下部にユーティリティラックを配置します。
- 右インスペクタは `トラック` / `作曲` / `ミックス`、下部ラックは `ミックス提案` / `ミックス履歴` / `作曲提案` / `A/B比較` / `作曲履歴` の5タブです。
...
6. DAWの簡易ピアノロールでノートをドラッグして微調整
7. `音階/楽器を反映` または `選択MIDI試聴` で確認
8. 必要に応じて `挿入を巻き戻し`
