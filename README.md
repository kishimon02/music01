...
現在は「実波形入力での特徴抽出」「C++音声コア経由の実音再生（WAV）」「再生同期/波形表示」「rule/LLM提案切替」まで実装しています。
...
- 提案エンジンは環境変数 `MUSIC_CREATE_SUGGESTION_ENGINE`（`rule-based` / `llm-based`）で指定可能です
- `llm-based` 利用時は `MUSIC_CREATE_LLM_ENDPOINT`（任意で `MUSIC_CREATE_LLM_API_KEY`）を設定してください
...
6. `提案エンジン` で `ルールベース` / `LLMベース` を選択する（既定はルールベース）
7. `提案` でミキシング候補を比較する（LLM失敗時はルールベースへ自動フォールバック）
8. 再生中はプレイヘッドと波形カーソルが同期して進む
9. `試聴` / `試聴取消` / `適用` / `選択を巻き戻し` で提案を運用する
...
3. 提案は `rule-based` が既定で、`llm-based` 選択時に失敗すると自動で `rule-based` にフォールバックします。
