...
1. 操作領域は単一ワークスペースとし、上部トランスポート/ヘッダー、中央編集領域、右インスペクタ、下部ユーティリティラックで構成する
2. 右インスペクタは `トラック` / `作曲` / `ミックス` の3タブ固定とする
3. 下部ユーティリティラックは `ミックス提案` / `ミックス履歴` / `作曲提案` / `A/B比較` / `作曲履歴` の5タブとする
...
2. アレンジャー領域にタイムライン、トラックレーン、小節グリッド、クリップ表示、WAV overview をまとめる
3. エディタ領域に音階表示、選択クリップ概要、簡易ピアノロールをまとめる
4. トラック追加、MIDI/オーディオクリップ追加
5. 選択MIDIの手動編集（ノート単位ドラッグ）
6. 半音シフトによる音階変更
7. 楽器Program変更
8. 編集済みMIDIの即時試聴
